...
public class ForumScreenTest extends TestBase  {
	...
	String expecteduserstatusonpost="Stargate all access pass.Diehard Access Only Diehard Access Only - Opening Act";
	...
	String[][] userdata=new String[18][2];
		
		
		
    userdata=ExcelUtil.setExcelFile(ExcelUtil.testDataExcelPath, "sheet1");
    for(int i=0;i<5;i++){
    	for(int j=0;j<2;j++){
   // userdata[i][j]="Name";
  ...
  public void createpostvalidate() throws Exception {
   ...
   Thread.sleep(9000);
    ...
    //  jse.executeScript("document.getElementById('ssn').value='555-55-5555';");
      ...
      performAct.sendKeys(forumobj.placeholder1, "post by user to check auto572437524").build().perform();
  ...
  ele.sendKeys("/Users/kiwitech/Documents/allign.png");
	 ...
	 // actions2.build().perform();
        ...
        //   actualusernameonimage=forumobj.userprifileimageonpost.getAttribute("title");
         ...
         imagegotuploaded=forumobj.useruploadeimageonpost.isDisplayed();
       ...
       imagegotuploaded=false;	 
        
           }
           
           System.out.println(imagegotuploaded);

           AssertJUnit.assertEquals(true, imagegotuploaded);
